    ...
    <div className="space-y-4">
      <div className="flex items-center justify-end mb-4 space-x-4">
        ...
        <label className="relative inline-flex items-center cursor-pointer">
          ...
          <span className="ms-3 text-sm font-medium text-gray-500">立場を表示</span>
        </label>
      </div>
      ...
      {filteredComments.map((comment) => (
        <div key={comment._id} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          <div className="flex flex-col">
                  ...
                  <div className="group relative">
                    {comment.sourceUrl ? (
                      <a
                        ...
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium hover:opacity-80 ${getSourceTypeStyle(
                          comment.sourceType
                        )}`}
                      >
                      ...
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getSourceTypeStyle(
                          comment.sourceType
                        )}`}
                      >
                        {getSourceTypeName(comment.sourceType)}
            ...
            {/* 抽出結果の表示 */}
            <div className="mt-2">
              <p className="text-gray-700">{comment.extractedContent || "抽出結果なし"}</p>
            </div>
                      ...
                      return (
                        <div key={stance.questionId} className="group relative">
                          <div className="flex items-center gap-2 bg-blue-50 p-2 rounded-lg">
                            <span className="text-sm font-medium text-blue-800">{stanceName}</span>
